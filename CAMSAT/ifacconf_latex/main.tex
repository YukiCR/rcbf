...
\begin{abstract}                % Abstract of 50--100 words
   % Miniaturization and clustering exacerbate the risk of satellite collisions.
   In this paper, we propose a distributed framework for inter-satellite collision avoidance with tunable priority.
   We first deduce the collision-free constraint of multi-satellite systems based on High Order Control Barrier Function techniques. 
   We then decouple such a constraint for respective satellites, and further encode safety on the nominal controller in the form of distributed safety filter.
   ...
   We further propose two methods to decide the priority parameters: cooperating with Large Language Models to tune based on task and cooperating with optimization to approximate global optimal behavior.
   Theoretical analysis have proved the safety guarantee of the safety filter and numerical simulations validated the effectiveness of proposed method.
\end{abstract}

\begin{keyword}
Multi-satellite, Collision Avoidance, Control Barrier Functions
\end{keyword}
