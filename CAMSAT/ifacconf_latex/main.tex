...
\begin{remark}
   ...
   Since $\|\boldsymbol{v}_{ij}\|^2 -  (\hat{\boldsymbol{n}}_{ij}^{\top}\boldsymbol{v}_{ij} )^2  \ge 0$ holds universally and $d_{ij} - R_{ij} \ge 0$ holds as long as agent $i$ and agent $j$ do not collide with each other, the right hand side of (\ref{eqn:safeProtocol}) is positive correlated to $p_{ij}$.
   ...
   By tuning the priority matrix $\boldsymbol{P} = \{p_{ij}\} \in \left\{ \boldsymbol{P} \in \mathbb{R}^{N\times N} \mid p_{ii} = 0,  p_{ij} + p_{ji} \le 1, i\neq j \right\}$, the high-level decision is introduced into the distributed safety filter, deciding ``who should evading more to avoid collision''.
...
\par While the collsiion avoidance requirement is clear and easy to be modelled mathematiccally, the mission level requirement is often much more complicated and often described in natural language. 
...
In addition, structured output technique is adopted to force the output format to LLMs.
...
\begin{figure}[t!]
   ...
   \centering
   \subfloat[$p_{12} = 0.5, p_{21} = 0.5$]{\includegraphics[width=0.9\linewidth, trim = \leftClip{} \lowerClip{} \rightClip{} \upperClip{}, clip]{llm1.pdf} \label{fig:11}} \\
   \subfloat[$p_{12} = 0.7, p_{21} = 0.3$]{\includegraphics[width=0.9\linewidth, trim = \leftClip{} \lowerClip{} \rightClip{} \upperClip{}, clip]{llm2.pdf} \label{fig:22} } \\
   \subfloat[$p_{12} = 1.0, p_{21} = 0.0$]{\includegraphics[width=0.9\linewidth, trim = \leftClip{} \lowerClip{} \rightClip{} \upperClip{}, clip]{llm3.pdf} \label{fig:33} } 
   \label{fig:111}
 \end{figure}

\begin{figure*}[t!]
   \newcommand{\upperClip}{10bp}
   \newcommand{\lowerClip}{0bp}
   \newcommand{\leftClip}{0bp}
   \newcommand{\rightClip}{0bp}
   \centering
   \subfloat[$p_{12} = 0.5, p_{21} = 0.5$]{\includegraphics[width=0.45\linewidth, trim = \leftClip{} \lowerClip{} \rightClip{} \upperClip{}, clip]{opt1.pdf} \label{fig:}} \hfil
   \subfloat[$p_{12} = 0.7, p_{21} = 0.3$]{\includegraphics[width=0.45\linewidth, trim = \leftClip{} \lowerClip{} \rightClip{} \upperClip{}, clip]{opt2.pdf} \label{fig:} } \\
   \subfloat[$p_{12} = 1.0, p_{21} = 0.0$]{\includegraphics[width=0.45\linewidth, trim = \leftClip{} \lowerClip{} \rightClip{} \upperClip{}, clip]{opt3.pdf} \label{fig:} } \hfil
   \subfloat[$p_{12} = 0.5, p_{21} = 0.5$]{\includegraphics[width=0.45\linewidth, trim = \leftClip{} \lowerClip{} \rightClip{} \upperClip{}, clip]{opt4.pdf} \label{fig:}} 
   \caption{Agents' behavior with different priority parameters.}
   \label{fig:}
 \end{figure*}
