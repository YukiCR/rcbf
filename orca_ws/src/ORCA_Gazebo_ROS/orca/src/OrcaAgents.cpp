...
/* @brief set the reference and real velocity for the simulator*/
...
/* @brief set the velocity in the simulator as the setpoint for gazebo (cmd_vel) */
void getOrcaVelocities(RVO::RVOSimulator *sim, std::vector<OrcaAgent*> agents,ros::Publisher new_vel_pb){
	sim->doStep(); // step forward
...
/* @brief set the states in the simulator with values in the gazebo */
void updateORCA(RVO::RVOSimulator *sim, std::vector<OrcaAgent*> agents){
	...
	for(int i=0;i<sim->getNumAgents();i++){
		...
		agents[i]->updateGoal(); // set a new goal if arrived
...
{		
	...
	do{
		...
		/* the following four lines are just for visualization */
